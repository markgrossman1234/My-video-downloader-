...
SESSION = os.getenv("SESSION")              # your user session string
...
# Link handler (runs alongside debug; if text is a /c/ link, this kicks in)
...
# =========================
async def main():
    # Start both clients ONCE; do NOT use 'async with user' inside handlers.
    await bot.start()
    await user.start()
...
if __name__ == "__main__":
    # Start tiny web server in a background thread for Render port check
